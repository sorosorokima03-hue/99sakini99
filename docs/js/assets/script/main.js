  ...
  scene.onLoad.add(() => {
    ...
    class Tile extends g.E {
      constructor(id, gx, gy, gz) {
        ...
        this.update.add(() => {
          this.highlight.opacity = this.isSelected ? (Math.sin(g.game.age / 4) + 1) / 2 * 0.4 + 0.2 : 0;
    ...
    // --- メインループ (Loading処理 + ゲームループ) ---
    const updateHandler = () => {
      // ★ LOADINGステップ処理 (フリーズ対策)
      if (loadingStep > 0) {
        switch (loadingStep) {
          case 1:
            // アセット変数確保 & UI初期化
          ...
          case 4:
            ...
            toggleGameUI(true);
            startBGM();
            loadingStep = 0; // Loading完了、通常ループへ
            break;
        }
        return; // Loading中はゲーム時間を進めない
      }

      // ★ エラー修正: ゲーム開始前はここでストップ
      if (!isGameStarted) return;
